#!/usr/bin/env python3
"""Create the second targeted qualitative audit sample."""
...
def export_sample(sample: pd.DataFrame, tsv_path: Path, csv_path: Path) -> None:
    out = sample.copy()
    out.insert(0, "audit_id", [f"QB{index:03d}" for index in range(1, len(out) + 1)])
    for column in [
        "human_isocolonic",
        "formal_parallelism",
        "rhetorically_relevant",
...
def write_rubric(path: Path, sample_tsv: Path) -> None:
    path.write_text(
        "\n".join(
            [
                "# Qualitative Isocolon Audit Batch 2",
                "",
                f"Sample: `{sample_tsv}`",
                "",
                ...
                "",
                "This batch targets the measurement boundary: high and mid broad targets,",
                "narrow antithesis/contrast, dialogue echo candidates, comparative",
                "concession, high-scoring non-target artifacts, and restatement controls.",
                "",
            ]
        ),
        encoding="utf-8",
...
def main() -> None:
    ...
    parser.add_argument("--exclude", action="append", default=["outputs/audit/isocolon_qualitative_audit_40.csv"])
    parser.add_argument("--out-tsv", default="outputs/audit/isocolon_qualitative_audit_batch2_40.tsv")
    parser.add_argument("--out-csv", default="outputs/audit/isocolon_qualitative_audit_batch2_40.csv")
    parser.add_argument("--rubric", default="outputs/audit/qualitative_audit_batch2_rubric.md")
